...
const canvascontext: CanvasRenderingContext2D = canvas.getContext(
    "2d"
...
function update() {
    ...
    function createGreenWall(x:number, y:number, w:number, h:number) {
        if (canvascontext && canvas) {
    ...
    function createYellowWall(x:number, y:number, w:number, h:number) {
        if (canvascontext && canvas) {
...
function specialWalls() {
    if (canvascontext && canvas) {
        ...
        if (
            ...
            y1_velocity_d = 0;
            y1_velocity_u = 0;
            const myH1tag: HTMLElement | null = document.getElementById(
                "h1"
            ) as HTMLElement;
            if (myH1tag) {
